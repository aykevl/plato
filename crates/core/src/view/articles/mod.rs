...
use crate::articles::{self, wallabag};
...
impl Articles {
    ...
    fn update_shelf(&mut self, rq: &mut RenderQueue, context: &mut Context) {
        ...
        self.articles = context
            .article_service
            .filter(context.settings.article_list);
    ...
    fn handle_event(
        ...
        match *evt {
            Event::Select(EntryId::LoginWallabag) => {
                let view = AccountWindow::new(context);
            ...
            Event::Select(EntryId::Logout) => {
                ...
                context.article_service = articles::load(ArticleAuth::default());
                self.update_shelf(rq, context);
                true
            }
            Event::Select(EntryId::Update) => {
                if !context.article_service.update(hub) {
            ...
            Event::ArticlesAuth(ref result) => {
                match result {
                    Ok(auth) => {
                        ...
                        context.article_service =
                            articles::load(context.settings.article_auth.clone());
                        context.article_service.update(hub);
                    }
                    Err(msg) => {
