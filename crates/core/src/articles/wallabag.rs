...
use crate::{
    articles::{read_index, Article, ArticleIndex, Changes, Service, ARTICLES_DIR},
    settings::{ArticleAuth, ArticleList},
...
impl Wallabag {
    pub fn load(auth: ArticleAuth) -> Wallabag {
...
impl Service for Wallabag {
    fn filter(&self, list: crate::settings::ArticleList) -> Vec<super::Article> {
        // TODO: perhaps only return a list of articles on the current page, to
        // reduce the amount of cloning?
        let mut articles: Vec<Article> = self.index
            .lock()
            .unwrap()
            .articles
            .values()
...
fn update(hub: &Hub, auth: ArticleAuth, index: Arc<Mutex<ArticleIndex>>) -> Result<(), Error> {
    ...
    match std::fs::create_dir(ARTICLES_DIR) {
        Ok(()) => {}
        Err(err) => {
            if err.kind() != ErrorKind::AlreadyExists {
                return Err(Error::other(format!(
                    "could not create articles directory: {err}"
                )));
            }
        }
    };
